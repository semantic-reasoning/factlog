...
| **Python CLI command** | Inside a Claude Code session (`!` prefix) | `factlog status`, `factlog review`, `factlog accept`, `factlog reject`, `factlog amend` | Check KB status, and let **a human** review, accept, retire, or correct candidate facts. `accept`/`reject` are the **human gate** that confirms a candidate as accepted or retires it (not an automated step). |
...
confirmation as accepted goes through the CLI gate a human runs in that same
session with `!` (`!factlog accept` / `!factlog reject` /
`!factlog amend --accept`). Only an
