...
기존 KB 에 전각 amount compound term(`amount(１００,"억")`)이 저장돼 있다면,
따옴표 없이 쓴 `amount(１００,억)` 질의는 여전히 미스합니다 — 전각 항은 더 이상
유효한 amount 가 아니라서 저장본과 같은 정준형으로 접히지 않기 때문입니다. 다만
`ask_router`의 `validate`·`render`와 0행 `evaluate`는 원문 숫자 코드포인트까지 같은
저장 표기를 찾았을 때 stderr에 이 레거시 단위-따옴표 근접 표기를 경고합니다. stdout
JSON, 경로, 행 수와 종료 코드는 바뀌지 않으며 NFKC/호환성 접기나 재시도도 하지
않습니다. 숫자·숫자 체계·단위가 다르거나 일반 식별자에 숫자가 든 경우에는 이 경고가
나오지 않습니다.
...
파싱된 스칼라로 묶으며, 주어·관계와 타입 미지정 객체는 NFC 동치로 묶습니다. 따라서
`amount(5400,"억")` 과 `amount(0.54,"조")` 는 세 곳 모두 한 값이고, ASCII/전각
쌍은 전각 쪽이 파싱되지 않아 세 곳 모두 서로 다른 값입니다. 관계 그룹핑과 일반
엔진 `relation/3` 원자 정체성은 NFC 동치를 하나로 보며, 출력은 실제 작성 표기에서
대표를 복원합니다. 의미적 별칭은 raw atom에서는 별도이고 optional `canonical/3`
블록에서 만납니다.
