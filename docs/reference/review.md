...
`tools/validate.py`는 `decisions/open-questions.md`의 다음 네 줄을 정확한 제목으로
요구합니다. 검증은 파일을 수정하지 않습니다.
...
파일이 없다면 `<KB_PATH>`를 실제 KB 경로로 바꿔 다음 명령을 실행하세요.

```bash
factlog init --target <KB_PATH> --no-activate
```

이 명령은 기존 파일을 덮어쓰거나 활성 KB 설정을 바꾸지 않지만, 다른 누락된
스캐폴드 파일도 함께 만들 수 있습니다. 파일이 이미 있는데 제목 일부가 없거나
이름이 바뀌었다면 `init`은 고치지 않습니다. 기존 설명과 목록 항목을 보존하면서
동일한 뜻의 제목은 위의 정확한 제목으로 되돌리고, 실제로 없는 제목만 추가한 뒤
검증을 다시 실행하세요.
