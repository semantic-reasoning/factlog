...
factlog where                             # 활성 KB가 ./factlog-demo 인지 확인
```
...
| `policy/questions.md` | 자연어 연구 질문 목록(q1–q4). |
...
factlog status    # → facts: 8 ... [accepted=1, confirmed=7]; 8 engine fact(s)
```
...
이 예제의 `query.dl` 은 일부러 **hit 과 miss 를 모두** 시연합니다.
...
- q5 는 accepted 관계로 답할 수 없어 `review_required` 로 사람에게 라우팅됩니다.
...
실습이 끝나면 복사본은 지워도 됩니다(원본은 영향 없음).
