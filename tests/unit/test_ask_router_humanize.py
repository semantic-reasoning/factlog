...
class TestRenderEngineAnswerHumanize:
    def test_compound_object_gets_annotation(self):
        ...
        out = ask_router.render_engine_answer(
            "relation(을서비스, 정식운영, X)", [row], signals=None, annotate_objects=True
        )
        assert "(= 7억)" in out
    ...
    def test_compound_date_gets_annotation(self):
        ...
        out = ask_router.render_engine_answer(
            "relation(프로젝트, 완료일, X)", [row], signals=None, annotate_objects=True
        )
    ...
    def test_compound_number_gets_annotation(self):
        ...
        out = ask_router.render_engine_answer(
            "relation(항목, 수치, X)", [row], signals=None, annotate_objects=True
        )
        assert "(= 2.5)" in out
    ...
    def test_annotation_gate_off_by_default(self):
        # A 3-element row rendered without annotate_objects=True (the non-relation
        # path default) must NOT gain any "(= ...)" annotation, proving the gate.
        row = ["factlog", "budget_exceeded", 'amount(7,"억")']
        out = ask_router.render_engine_answer(
            'flagged(a, b, c)', [row], signals=None, annotate_objects=False
        )
        assert "(= " not in out

    def test_annotation_gate_on_with_flag(self):
        # The same 3-element row WITH annotate_objects=True DOES get the annotation,
        # pinning that the flag is the sole gate.
        row = ["factlog", "budget_exceeded", 'amount(7,"억")']
        out = ask_router.render_engine_answer(
            'flagged(a, b, c)', [row], signals=None, annotate_objects=True
        )
        assert "(= 7억)" in out
