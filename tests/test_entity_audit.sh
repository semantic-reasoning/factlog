...
printf '%s\n%s\n%s\n%s\n' "$H" \
  '코리봇,통합_대상,화성in,sources/a.md,accepted,0.9,' \
  '코리봇,통합_대상,화성 소통봇,sources/a.md,accepted,0.9,' \
  '화성in,정식_운영,2026.4,sources/a.md,accepted,0.9,' > "$KB/facts/candidates.csv"
printf 'x\n' > "$KB/sources/a.md"
# add a substring-contained pair for fragmentation
printf '%s\n' '소버린 플랫폼,예시,경기도 소버린 플랫폼,sources/a.md,accepted,0.9,' >> "$KB/facts/candidates.csv"

run
[ "$rc" -eq 0 ] && ok "populated audit exits 0" || bad "exit $rc"
printf '%s' "$out" | grep -qE "\[ *2\] 코리봇" && ok "entity list reports fact count" || bad "entity fact count missing"
printf '%s' "$out" | grep -qF "'경기도 소버린 플랫폼' ⟷ '소버린 플랫폼'" && ok "substring fragmentation candidate surfaced" || bad "fragmentation candidate missing"
printf '%s' "$out" | grep -qF "relation '정식_운영' has literal-looking object(s): 2026.4" && ok "literal suspect under undeclared relation surfaced" || bad "literal suspect missing"

# the motivating non-date literal forms (ordinal '1호 공약', year '2026년', amount '100억')
printf '%s\n%s\n%s\n' \
  '코리봇,공약_순위,1호 공약,sources/a.md,accepted,0.9,' \
  '화성in,운영_연도,2026년,sources/a.md,accepted,0.9,' \
  '예산,규모,100억,sources/a.md,accepted,0.9,' >> "$KB/facts/candidates.csv"
run
for v in "1호 공약" "2026년" "100억"; do
...
printf '%s' "$out" | grep -qF "2026.4  (accepted)" && ok "undeclared literal still listed as entity" || bad "literal not listed as entity"
...
# 2026.4 should now be under declared literals, not flagged as a suspect
