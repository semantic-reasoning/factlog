...
printf '%s\n%s\n%s\n' "$H" \
  '화성in,정식_운영,2026.4,sources/a.md,accepted,0.9,' \
  '코리봇,통합_대상,화성in,sources/a.md,accepted,0.9,' > "$KB/facts/candidates.csv"
printf 'x\n' > "$KB/sources/a.md"
...
print("OK" if (c.attribute_relations() == set() and "2026.4" in es and es == vs)
...
es, vs = c.entity_set(facts), c.value_set(facts)
checks = {
    "literal out of entity_set": "2026.4" not in es,
    "literal still in value_set": "2026.4" in vs,
    "entity object retained": "화성in" in es,
    "subject retained": "코리봇" in es,
}
...
cases = {
    "relation w/ literal object -> ok":
        c.classify_query('relation("화성in","정식_운영","2026.4")?', facts, P)[1] == c.QUERY_OK,
    "relation w/ entity object -> ok":
        c.classify_query('relation("코리봇","통합_대상","화성in")?', facts, P)[1] == c.QUERY_OK,
    "path naming literal -> rejected":
        c.classify_query('path("화성in","2026.4")?', facts, P)[1] == c.QUERY_ENTITY_NOT_ACCEPTED,
    "relation w/ unknown object -> rejected":
        c.classify_query('relation("화성in","정식_운영","9999.9")?', facts, P)[1] == c.QUERY_ENTITY_NOT_ACCEPTED,
}
