...
> `/factlog ...` slash command로 쓰고, 검토·승인 같은 사람의 게이트는 같은 세션에서
> `!` 를 앞에 붙여 Python CLI(`!python3 -m factlog ...`)로 사람이 직접 실행합니다. 두
> 입구 모두 같은 결정론 엔진을 호출합니다 — slash command · Python CLI · 검증 엔진,
> 이 셋이 한 도구입니다.
