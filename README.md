...
- Claude Code CLI
- **git** — 마켓플레이스 설치가 내부적으로 `git clone`을 사용합니다. Windows에서는 **Git for Windows**를 설치하세요(= Git Bash 제공).
...
실행됩니다. **Git for Windows**를 설치하면 Git Bash가 함께 제공되어, 이 절의
bash 명령이 그 위에서 동작합니다.
