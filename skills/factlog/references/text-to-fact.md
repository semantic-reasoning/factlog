...
**주의(직렬화).** `amount(N,"단위")`는 따옴표를 포함하므로, 현재 엔진의 `.dl` 텍스트
파서가 `accepted.dl`의 escape된 따옴표를 거부해 프로그램을 깨뜨립니다(이슈 #154).
그래서 비교가 필요한 금액은 따옴표 없는 **`number(<기준단위 정수>)`** 로 적는 편이
안전합니다(예: `126백만원` → `number(126000000)`).
