...
- **`factlog ingest`로 자동 변환**: `.docx`, 바이너리 `.pdf`, `.odt`, `.epub`, `.html`, `.rtf`(pandoc·textutil·pdftotext), 그리고 `.hwpx`(한컴 OWPML — 내장 추출기, 외부 도구 불필요). 변환본은 `runs/sources/`에 기록되어 추출에 쓰입니다.
- **직접 미지원(수동 변환 필요)**: `.pptx`, `.xlsx`, `.hwp`(구 바이너리 — `.hwpx`/`.docx`/`.pdf`로 내보낸 뒤 ingest), 이미지. `sources/`에 그대로 두면 source 경로로만 등록되고 fact는 생성되지 않습니다(조용한 미인제스트).
