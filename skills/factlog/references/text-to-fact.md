...
**엔진 지원 현황(과대약속 금지).** `date`·`ordinal`은 비교 가능한 스칼라로 투영
됩니다(완전 지원). `amount`는 단위표가 필요하고 **양의 정수만** 받으므로, 음수
금액(예: 적자 영업이익)은 `amount`가 아니라 `number(-672)`로 둡니다. `number`
투영은 추가 구현(이슈 #125)이 남아 있어 현재는 일치 질의까지만 보장됩니다 —
그래도 구조 보존을 위해 컴파운드 텀으로 적습니다.
